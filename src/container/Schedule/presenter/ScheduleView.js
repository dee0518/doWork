...
function ScheduleView(props) {
  ...
  const onClickAddBtn = () => {
    ...
    let month =
      pre.getMonth() + 1 > 9 ? pre.getMonth() + 1 : "0" + (pre.getMonth() + 1);
    let date = pre.getDate();
    let hours = pre.getHours() > 9 ? pre.getHours() : "0" + pre.getHours();
    let endHours =
      pre.getHours() + 1 > 9 ? pre.getHours() + 1 : "0" + (pre.getHours() + 1);
    let minutes =
      Math.ceil(pre.getMinutes() / 10) * 10 >= 60
        ? "00"
        : Math.ceil(pre.getMinutes() / 10) * 10;
  ...
  const onClickModalBtn = (e) => {
    ...
    if (
      (newSchedule.title !== "" && newSchedule.category !== "") ||
  ...
  const onClickSubTitle = (title) => {
    ...
    if (Object.keys(params).length === 0) {
      url = `${title}/month/${today.getFullYear()}/${
        today.getMonth() + 1
      }/${today.getDate()}`;
    } else {
  ...
  const onAddParticipants = () => {
    if (newSchedule.participants.length < 8) {
      ...
      setNewSchedule({ ...newSchedule, participant: "" });
    }
  };
  ...
  const onDeleteParticipant = (name) => {
    ...
    setNewSchedule({ ...newSchedule, participants: filterGroup });
  };
  ...
  const onChageCheckList = (e) => {
    ...
    let restChk = checkList.filter(
      (v, i) => i !== 0 && i != id - 1 && v.checked === true
    ).length;
  ...
  useEffect(() => {
    ...
    let url;
    if (Object.keys(params).length === 0) {
      url = `calendar/${selectValue}/${today.getFullYear()}/${
        today.getMonth() + 1
      }/${today.getDate()}`;
    } else {
    ...
    <Wrapper className="schedule-view-wrapper">
      ...
      <Wrapper className="search-cal-group">
        <Wrapper className="top-group">
          <SearchForm value={searchValue} onChange={onChangeSearch} />
          <Button className={"alarm-btn"} onClick={onClickAlarmBtn}>
            Alarm
          </Button>
        </Wrapper>
